...
from app.menus.menus import API_MENU
from app.ui import (
    console,
    render_book_list,
    render_text_output,
...
def handle_fetch_by_ref(mode: str) -> dict | None:
    """
    Fetch verse, chapter, or random verse based on mode.

    Args:
        mode: 'v' for verse, 'c' for chapter, 'r' for random

    Returns:
        Dictionary with verse data or None if fetch failed
    """
    if mode == 'v':
...
def handle_fetch_multiple_books():
    ...
    estimated_time = (total_chapters_to_try * delay) / 60
