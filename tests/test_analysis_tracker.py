...
class TestCompareAnalyses:
    """Test comparing two analyses to find differences and similarities."""

    def test_compare_returns_common_words(self, tracker_with_user, sample_word_freq, sample_vocab_info):
        """Test that comparison identifies words present in both analyses."""
        tracker, user_id, db_path = tracker_with_user
        ...
        # Create second analysis with some overlapping words
        word_freq_2 = [("jesus", 150), ("love", 60), ("peace", 40), ("joy", 35)]
        id2 = tracker.save_word_frequency_analysis(
            word_freq=word_freq_2,
            vocab_info=sample_vocab_info,
            scope_type="query",
            scope_details={"query_id": "query_2"},
            verse_count=30
        ...
        assert "common_words" in comparison
        common_words = comparison["common_words"]
        assert "jesus" in [w[0] for w in common_words]
        assert "love" in [w[0] for w in common_words]
    
    def test_compare_returns_unique_words(self, tracker_with_user, sample_word_freq, sample_vocab_info):
        """Test that comparison identifies words unique to each analysis."""
        tracker, user_id, db_path = tracker_with_user
        ...
        word_freq_2 = [("jesus", 150), ("peace", 40), ("joy", 35), ("hope", 25)]
        id2 = tracker.save_word_frequency_analysis(
            word_freq=word_freq_2,
            vocab_info=sample_vocab_info,
            scope_type="query",
        ...
        # Words unique to first
        unique_first = [w[0] for w in comparison["unique_to_first"]]
        assert "lord" in unique_first
        assert "god" in unique_first
        assert "faith" in unique_first
        
        # Words unique to second
        unique_second = [w[0] for w in comparison["unique_to_second"]]
        assert "peace" in unique_second
        assert "joy" in unique_second
        assert "hope" in unique_second
    
    def test_compare_returns_frequency_differences(self, tracker_with_user, sample_word_freq, sample_vocab_info):
        """Test that comparison shows frequency changes for common words."""
        tracker, user_id, db_path = tracker_with_user
        ...
        word_freq_2 = [("jesus", 150), ("lord", 90), ("love", 60)]
        id2 = tracker.save_word_frequency_analysis(
            word_freq=word_freq_2,
            vocab_info=sample_vocab_info,
            scope_type="query",
        ...
        assert "frequency_changes" in comparison
        
        # Check that frequency differences are calculated
        freq_changes = comparison["frequency_changes"]
        jesus_change = next((f for f in freq_changes if f[0] == "jesus"), None)
        assert jesus_change is not None
        assert jesus_change[1] == 120  # Original count
        assert jesus_change[2] == 150  # New count
        assert jesus_change[3] == 30   # Difference
    
    def test_compare_returns_none_for_invalid_ids(self, tracker_with_user):
        """Test that comparing non-existent analyses returns None."""
        tracker, user_id, db_path = tracker_with_user

        comparison = tracker.compare_analyses("invalid_id_1", "invalid_id_2")

        assert comparison is None
    
    def test_compare_includes_metadata(self, tracker_with_user, sample_word_freq, sample_vocab_info):
        """Test that comparison includes metadata about both analyses."""
        tracker, user_id, db_path = tracker_with_user
        ...
        id1 = tracker.save_word_frequency_analysis(
            ...
            vocab_info=sample_vocab_info,
            scope_type="query",
            scope_details={"query_id": "John 1"},
            verse_count=25
        )

        word_freq_2 = [("jesus", 150), ("love", 60)]
        id2 = tracker.save_word_frequency_analysis(
            word_freq=word_freq_2,
            vocab_info=sample_vocab_info,
            scope_type="book",
            scope_details={"book": "Romans"},
            verse_count=50
        )

        comparison = tracker.compare_analyses(id1, id2)

        assert "analysis_1" in comparison
        assert "analysis_2" in comparison
        assert comparison["analysis_1"]["id"] == id1
        assert comparison["analysis_2"]["id"] == id2
        assert comparison["analysis_1"]["scope_type"] == "query"
        assert comparison["analysis_2"]["scope_type"] == "book"
